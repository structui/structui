...
export function ComponentDetailClient({ component }: ComponentDetailClientProps) {
  ...
  const cliInstallCode = component.installCode || `npx sui add ${component.slug}`;
  const registryDetailUrl = `/api/registry/components/${component.slug}`;
  const registryDownloadUrl = `/api/registry/download/component/${component.slug}`;
  const shadcnItemUrl = `/api/r/${component.slug}`;
  const apiInstallCode = `curl -s https://structui.com${registryDetailUrl} | jq '.files[0]'\ncurl -L https://structui.com${registryDownloadUrl} -o ${component.slug}.tsx`;
  const shadcnInstallCode = `npx shadcn@latest add https://structui.com${shadcnItemUrl}`;
  ...
  const copyInstall = () => {
    navigator.clipboard.writeText(cliInstallCode).then(() => {
            ...
            <TabsContent value="manual" className="mt-6 space-y-4">
              <Card className="border-border/70">
                <CardHeader>
                  ...
                  <CardDescription className="text-xs">
                    Install this component directly from the StructUI CLI.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-3">
                  <CodeBlock code={cliInstallCode} language="bash" />
                  <CodeBlock
                    code={`npx sui info ${component.slug}\nnpx sui search ${component.slug}`}
                    language="bash"
                  />
                </CardContent>
              </Card>
              ...
              <Card className="border-border/70">
                <CardHeader>
                  <CardTitle className="flex items-center gap-2 text-sm font-semibold">
                    ...
                    Registry API install
                  </CardTitle>
                  <CardDescription className="text-xs">
                    Inspect JSON metadata and download raw source from API routes.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-3">
                  <CodeBlock code={apiInstallCode} language="bash" />
                  <CodeBlock
                    code={`Component detail: ${registryDetailUrl}\nDirect download: ${registryDownloadUrl}`}
                    language="bash"
                  />
                </CardContent>
              </Card>
              ...
              <Card className="border-border/70">
                <CardHeader>
                  ...
                  <CardDescription className="text-xs">
                    Use the registry item endpoint with shadcn-compatible tooling.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-3">
                  <CodeBlock code={shadcnInstallCode} language="bash" />
                  <CodeBlock code={`Registry item: ${shadcnItemUrl}`} language="bash" />
                </CardContent>
              </Card>
              ...
              <Card className="border-border/70">
                <CardHeader>
                  ...
                  <CardDescription className="text-xs">
                    Copy-paste path for teams that want full source ownership.
                  </CardDescription>
                </CardHeader>
            ...
            <TabsContent value="docs" className="mt-6">
              <Card className="border-border/70">
                <CardHeader>
                  ...
                  <CardDescription className="text-xs">
                    Rendered from{" "}
                    <code className="rounded bg-muted px-1 py-0.5">docs/components</code>.
                  </CardDescription>
                </CardHeader>
                <CardContent>
